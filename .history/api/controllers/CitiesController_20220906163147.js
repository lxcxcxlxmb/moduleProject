...
class StatesController {
  ...
  _validateData = async (data, id) => {
    const attributes = ['name', 'province', 'sex'];
    const state = {};
    ...
    if (await this._checkIfStateExists(state.name, id)) {
      throw new Error(`The state of "${state.name}" already exists.`);
    }

    return state;
  }

  _checkIfStateExists = async (name, id) => {
    const where = {
      name: name
    };
