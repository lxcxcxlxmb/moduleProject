...
const showCityCreateBox = async () => {
    ...
    const data = states.data;
    console.log(data);
    let optionHtml = String('n');
    // data.forEach((element) => {
    //     this.optionHtml += String(`<option value="${element.id}">${element.province}</option> `);
    //     console.log(this.optionHtml);
    // })
    for (let i = 0; i < data.length; i++) {
        this.optionHtml += String(`<option value="${data[i].id}">${data[i].province}</option>`);
        console.log(this.optionHtml);
    }
    ...
    Swal.fire({
        ...
        html:
            ...
            '<select name="states" id="states" class="swal2-select">' +
            this.optionHtml +
            '</select>',
        focusConfirm: false,
        showCancelButton: true,
